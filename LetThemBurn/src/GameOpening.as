package
{
	...
	public class GameOpening extends MovieClip
	{
		...
		public function GameOpening()
		{
			...
			opening3.renderer.alpha=0;
			
			game.addChild(opening3);
			game.addChild(opening2);
			game.addChild(opening1);
			
			var fader3:FaderBehaviour=new FaderBehaviour();
			var fader2:FaderBehaviour=new FaderBehaviour(fader3);
			var fader1:FaderBehaviour=new FaderBehaviour(fader2);
			
			
			opening1.addBehaviour(fader1);
			opening2.addBehaviour(fader2);
			opening3.addBehaviour(fader3);
			fader1.startFade();
		...
		private function keydown(e:Event):void{
			if(opening3.renderer.alpha>=1){
				SoundEngine.singleton.playEnter();
				stage.removeEventListener(KeyboardEvent.KEY_DOWN,keydown);
				addEventListener(Event.ENTER_FRAME,exitingFunction);	
			}
			
			
		}
		
		private var exitingCount:int=0;
		private function exitingFunction(e:Event):void{
			if(exitingCount>20){
				removeEventListener(Event.ENTER_FRAME,exitingFunction);
				SoundEngine.singleton.stopOpeningMusic();
				dispatchEvent(new Event(EXIT));
			}
			opening3.renderer.alpha-=0.05;
			if(SoundEngine.singleton.openingMusicChannel.soundTransform.volume>0){
			 	SoundEngine.singleton.openingMusicChannel.soundTransform=new SoundTransform(SoundEngine.singleton.openingMusicChannel.soundTransform.volume-0.05);
			}else{
				SoundEngine.singleton.openingMusicChannel.soundTransform=new SoundTransform(0);
			}
			exitingCount++;
